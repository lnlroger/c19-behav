...
Same story, only with lower adjusted R_squared. 
...
* More trusting ones are more compliant
* More altruistic societies are more compliant
...
* Share of older folks DOES NOT correlate with death from covid - strange...
* wealthier countries have more deaths (not sure if this is intuitive)
* Countries that obey Rule of Law have lower cases

In general though, the results are weak and R^2 is bad. 
...
Better. What if we focus this simpler coefficient to wave 1?
...
Finally, we get an interesting result for a change. Higher compliance, leads to fewer deaths. 
But, the effect disappears when we include other behavioural variables... 
