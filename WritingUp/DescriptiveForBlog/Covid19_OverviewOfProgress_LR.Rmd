...
For a systematic comparison of policies across countries, this type of data of course needs further processing and aggregating. Given CoronaNet's ambition of digesting such a large number of events (and along many dimensions), this seems like a daunting task. In order to collapse the information into a single index, the authors employ Bayesian methods that essentially aim to quantify the 'cost' of any given type of measure, and from that 'cost' derive a level of policy activity.
...
Second, and perhaps more problematically, there seems to be little movement downwards. While most countries started relaxing restrictions in May and June, the index remains largely unaffected. And rather unintuituvely, the only distinct drops in the policy index happen in late February.
...
Again, let us start by looking at the building blocks of the index. In contrast to CoronaNet, OxCGRT does not provide a database of individual evens, but instead offers almost binary time series on individual policies, which essentially flag whether a given policy is in place or not in the country. There is also rudimentary information on the 'intensity' of the policy (calues between 1 and 4).

The graph below plots index for a very select number of countries (chosen at random, except US and Germany), along with colorful bands that indicate whether a given policy was in place:
...
![legend](legend_improvised.PNG){width=40%}
