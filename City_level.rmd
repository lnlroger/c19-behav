...
gwmp2<-gwmp3%>% filter(City %in% city100$City)%>%
...
This is probably we have some cities that get coded to the wrong Country, like Florida in Puerto Rico. 
Specifying filter in City and Country improves but not completely fixes (from 424 to 421 when it should be 419).
...
We go from `r length(unique(gwmp3$City))` cities to 420 and the total amount of observations goes down to 60 thousands from 260 thousands...
...
Let's see on a country/city level basis. 
...
#Check before SI kicked in perhaps. 
...
Let's repeat regression of weather, controlling for SI
