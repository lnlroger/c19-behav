...
use dplyr lag on Movement. 

```{r}
z<-dfl%>%
  filter(!is.na(StringencyIndex))%>%
  filter(Type=="Movement")%>%
  group_by(ID,Country,Date,StringencyIndex)%>%
  summarise(Move=mean(Value_lag_1d,na.rm=T),first_lag=first(Value_lag_1d),first=first(Value),StrinInd=first(StringencyIndex),StringIndMean=mean(StringencyIndex))
